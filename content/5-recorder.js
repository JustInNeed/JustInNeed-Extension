 ...
 *   export 는 글(pageId) 단위로 합치지만, 이벤트마다 segId 가 남아서
 *   오프라인에서 구간 경계(탭 전환·재방문)를 알 수 있다. 연속 틱 간 차분
 *   (스크롤 감속 등)은 segId 가 바뀌는 곳에서 리셋할 것.
 *
 * --- 내보내기 ---------------------------------------------------------------
  ...
  function buildMeta() {
    return {
      ...
      notes: [
        ...
        '한 페이지 = 같은 글(pageId)의 모든 구간을 합친 것. segId 가 바뀌는 곳이 ' +
        '구간 경계(탭 전환·재방문)이므로 연속 틱 간 차분은 거기서 리셋할 것.',
